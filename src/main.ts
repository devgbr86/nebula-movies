...
// ─── Home ─────────────────────────────────────────────────
async function loadRandomMovie(app: HTMLElement): Promise<void> {
  const heroMovie = app.querySelector<HTMLDivElement>(".hero-movie");
  if (!heroMovie) return;
  ...
  for (const id of shuffled) {
    ...
    const m   = await res.json() as OMDbDetail;

    if (m.Response !== "True") continue;
    if (!m.Genre?.toLowerCase().includes("sci-fi")) continue;
